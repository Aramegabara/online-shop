...
from .models import Category,  Customer, Cart, CartProduct, Product#, Notebook, Smartphone, LatestProducts,
...
class ProductDetailView(CartMixin, DetailView):#CategoryDetailMixin,
...
class AddToCartView(CartMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        product_in_cart = Product.objects.get(slug=product_slug) # content_type.model_class()
        cart_product, created = CartProdcart_productuct.objects.get_or_create(
            user=self.cart.owner, cart=self.cart, product=product_in_cart #, content_type=content_type, object_id=product_in_cart.id
        )
        if created:
